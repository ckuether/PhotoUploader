...
class CameraFragment: Fragment(), ActivityCompat.OnRequestPermissionsResultCallback, View.OnClickListener {
    ...
    private val onImageAvailableListener = ImageReader.OnImageAvailableListener {
        val savedImage: Image = it.acquireNextImage()

        val buffer: ByteBuffer = savedImage.planes[0].buffer
        val bytes = ByteArray(buffer.remaining())
        buffer.get(bytes)
        var savedBitmap: Bitmap = BitmapFactory.decodeByteArray(bytes, 0, bytes.size, null)

        val rotation: Float = 90f //For some reason pictures aren't stored in portrait orientation
        val matrix = Matrix()
        matrix.preRotate(rotation)

        savedBitmap = Bitmap.createBitmap(savedBitmap, 0, 0, savedBitmap.width, savedBitmap.height, matrix, true)

        backgroundHandler?.post(ImageSaver(savedImage, file))
        mCallback.updateSavedBitmap(savedBitmap)
    }
    ...
    companion object {
        ...
        private lateinit var mCallback: PhotoUploaderCallback
        @JvmStatic fun newInstance(callback: PhotoUploaderCallback): CameraFragment{
            mCallback = callback
